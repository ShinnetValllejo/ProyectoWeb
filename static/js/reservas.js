...
document.addEventListener('DOMContentLoaded', async e => {
    ...
    const res = await fetch(`http://shineapi.us-east-2.elasticbeanstalk.com/user/${id}/my-booked-apartments`);
    await res.json()
    .then((data) => {
        ...
        data.map( reserva => {
            ...
            <div class="card-horizontal" style="align-items: center; gap:10px">
                ...
                <div class="card-body">
                    ...
                    <br>
                    <a onclick="deleteBook('${reserva._id}','${reserva.client._id}')" class="btn btn-danger">Eliminar Reserva</a>                      
                </div>
            </div>
        </div>
        <br>`
